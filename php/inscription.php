<?php 
	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		if(empty($error)){
			...
			if(isset($user)){
				User::connect($user->mail, $user->pass);
				if(User::isConnected()){
					...
					<form class="form" action="#" method="POST" role="form" autocomplete="off">
						<div class="container ">
							<div class="form-group row justify-content-between">
								...
								<input type="text" class="form-control col-sm-9" id="inputName" placeholder="full name" name="name" required>
							</div>

							<div class="form-group row justify-content-between">
								<label for="inputPseudo" class="col connectLabel pl-2 pr-2 mt-1 mr-3">Pseudo :*</label>
								<input type="password" class="form-control col-sm-9" id="inputPseudo" placeholder="pseudo" name="nickname" required>
							</div>

							<div class="form-group row justify-content-between">
								<label for="inputEmail" class="col connectLabel pl-3 pr-3 mt-1 mr-3">Email :*</label>
								<input type="email" class="form-control col-sm-9" id="inputEmail" placeholder="email@gmail.com" name="email" required>
							</div>

							<div class="form-group row justify-content-between">
								<label for="inputPassword" class="col connectLabel pl-2 pr-2 mt-1 mr-3">Mdp :*</label>
								<input type="password" class="form-control col-sm-9" id="inputPassword" placeholder="password" name="password" required>
							</div>

							  <!--Bouton s'inscrire-->
							<div class="form-group d-flex justify-content-around mt-2 mb-0">
								<button type="button" class="btn btn-secondary rounded-pill" data-dismiss="modal" aria-label="Close">Retour</button>
